...
import sys
sys.path.append(str(Path(__file__).resolve().parent.parent / 'm2m_segment-anything'))
from m2m_segment_anything import sam_model_registry
...
class sam_m2m(nn.Module):
    def __init__(self, seg, m2m):
        ...
        if seg == 'sam_vit_b':
            self.seg_model = sam_model_registry['vit_b'](checkpoint=DIR / 'm2m_segment-anything' / 'checkpoints' / 'sam_vit_b_01ec64.pth')
        elif seg == 'sam_vit_l':
            self.seg_model = sam_model_registry['vit_l'](checkpoint=DIR / 'm2m_segment-anything' / 'checkpoints' / 'sam_vit_l_0b3195.pth')
        elif seg == 'sam_vit_h':
            self.seg_model = sam_model_registry['vit_h'](checkpoint=DIR / 'm2m_segment-anything' / 'checkpoints' / 'sam_vit_h_4b8939.pth')
        self.seg_model.eval()
